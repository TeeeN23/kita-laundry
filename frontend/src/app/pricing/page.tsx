...
import { Truck, ChevronDown, ChevronUp, Loader2, Phone } from "lucide-react";
    ...
    try {
      ...
      if (data.success && data.items) {
        ...
        data.items.forEach((item: any) => {
          ...
          if (
            item.service === "dry_clean" ||
            item.service === "premium_dry_clean"
          ) {
            grouped[cat][itemName].dryClean = item.basePrice;
          } else if (
            item.service === "steam_press" ||
            item.service === "premium_steam_press"
          ) {
    ...
    <div className="mb-16">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          Transparent pricing for all our services. Select a category to view
          detailed pricing.
        </p>
      </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="text-center py-4 px-4 font-semibold text-gray-700 border-b">
                  Wash & Fold
                </th>
                <th className="text-center py-4 px-4 font-semibold text-gray-700 border-b">
                  Wash & Iron
                ...
                pricingData[activeCategory].map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-center text-gray-600">
                      {item.washFold > 0 ? `₹${item.washFold}` : "-"}
                    </td>
                    <td className="py-4 px-4 text-center text-gray-600">
                      {item.washIron > 0 ? `₹${item.washIron}` : "-"}
                    </td>
                    <td className="py-4 px-4 text-center text-gray-600">
                      {item.dryClean > 0 ? `₹${item.dryClean}` : "-"}
                    </td>
                    <td className="py-4 px-4 text-center text-gray-600">
                      {item.steamPress > 0 ? `₹${item.steamPress}` : "-"}
                    </td>
                  </tr>
                ))
              ) : (
  ...
  const faqData = [
    {
      ...
      answer:
        "No, pickup and delivery are completely free for all orders above ₹200. For orders below ₹200, a nominal charge of ₹30 applies.",
    },
    {
      question: "What payment methods do you accept?",
      answer:
        "We accept Cash on Delivery (COD), UPI payments, credit/debit cards, and digital wallets like Paytm, PhonePe, and Google Pay.",
    },
    {
      question: "Is there a minimum order value?",
      answer:
        "No minimum order value required! Orders above ₹200 qualify for free pickup and delivery.",
    },
    {
      question: "Do you offer same-day service?",
      answer:
        "Yes! Our Express Service provides same-day pickup and delivery. An additional express charge applies.",
    },
    {
      question: "What if I'm not satisfied with the service?",
      answer:
        "We offer a 100% satisfaction guarantee. If not satisfied, we'll redo your order for free or provide a full refund.",
    },
  ];
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-16 container mx-auto px-4">
        ...
        <p className="text-center text-lg text-gray-700 font-medium mt-8">
          * Prices may vary based on fabric type and condition. Final price
